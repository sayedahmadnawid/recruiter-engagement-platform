export default function CandidateEducations({ education = [] }) {
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center gap-2 mb-6">
        <AcademicIcon className="w-5 h-5 text-indigo-600" />
      ...
      {education.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {education.map((item, idx) => {
            ...
            const institution = item.institution || item.school || item.university || 'Institution Not Specified';
            const degree = item.degree || item.field_of_study || item.major || 'Degree / Certificate';
            const year = item.year || item.graduation_year || item.dates || formatDates(item.start_date, item.end_date);
              ...
              >
                ...
                {(year || gpa) && (
                  <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-200/60 text-xs text-gray-500">
                    <span>{year}</span>
                    {gpa && <span className="font-medium text-gray-700 bg-gray-200/50 px-2 py-0.5 rounded">{gpa}</span>}
                  </div>
                )}
              </div>
            );
